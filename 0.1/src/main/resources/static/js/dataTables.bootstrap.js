...
$.extend(true, $.fn.dataTable.defaults, {
	...
	"fnInitComplete": function (oSettings, json) {
		...
		if (currentId) {
			...
			var icongroup=$(' <div></div>').append(thisFilterInput);
...
$.extend( $.fn.dataTableExt.oPagination, {
	"bs_normal": {
		"fnInit": function( oSettings, nPaging, fnDraw ) {
			...
			$(nPaging).append(
				'<ul class="pagination">'+
				'<li class="prev disabled"><a href="javascript:void(0)"><i class="ion-arrow-left-b"></i></a></li>'+
				'<li class="next disabled"><a href="javascript:void(0)"><i class="ion-arrow-right-b"></i></a></li>'+
				'</ul>'
			);
			var els = $('a', nPaging);
		...
		"fnUpdate": function ( oSettings, fnDraw ) {
			...
			for ( i=0, ien=an.length ; i<ien ; i++ ) {
				...
				for ( j=iStart ; j<=iEnd ; j++ ) {
					sClass = (j==oPaging.iPage+1) ? 'class="active1"' : '';
		...
		"fnUpdate": function ( oSettings, fnCallbackDraw )
		{
			...
			for ( i=iStartButton ; i<=iEndButton ; i++ )
			{
				sList += (iCurrentPage !== i) ?
					...
					'<li class="active1"><a tabindex="'+oSettings.iTabIndex+'">'+oSettings.fnFormatNumber(i)+'</a></li>';
			}
